...
\title{Methodological Perspectives on EEG-Based Emotion and Mental State Recognition Using Signal Processing and Deep Learning}
...
\begin{abstract}
This review examines developments in electroencephalography (EEG)-based emotion and mental state recognition, focusing on the integration of digital signal processing and machine learning methodologies. The reviewed studies identify four dominant methodological patterns: time–frequency image conversion with convolutional neural network (CNN) feature extraction; hybrid CNN–LSTM architectures for joint spatial–temporal modeling; entropy-based and block-structured convolutional networks for continuous state decoding; and neuromorphic systems utilizing spiking neural networks (SNNs) for efficient spike-encoded classification.

Across these approaches, persistent challenges arise, including high inter-subject variability, signal noise and non-stationarity, methodological inconsistencies that hinder comparability, and the computational complexity of deep learning architectures. These limitations affect generalization, reproducibility, and the feasibility of real-time deployment, highlighting the need for standardized preprocessing workflows, robust evaluation protocols, and compact models suitable for embedded or wearable devices.

Solutions identified in the reviewed literature include data augmentation, transfer learning, domain adaptation, and multimodal fusion to enhance generalization and efficiency. Future research should emphasize the integration of these strategies into unified, interpretable pipelines that balance model performance with transparency and computational efficiency, enabling reliable and reproducible EEG-based emotion recognition systems for real-world applications.
\end{abstract}
